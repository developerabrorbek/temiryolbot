...
const choose_language = async msg => {
    ...
    if (!checkUser)
        await new User({chat_id,action:'choose_language'}).save()
    else 
        await User.findByIdAndUpdate(checkUser._id,{...checkUser,action:'choose_language'},{new:true})
...
const get_language = async msg => {
    ...
    let message 
    user.language = msg.text == '🇷🇺 Русский язык' ? 'ru' :
    msg.text == "🇺🇿 O'zbek tili" ? 'uz' : null

    if (!user.language){
        message = await bot.sendMessage(chat_id,
            `Assalom aleykum, ${msg.from.first_name}. Iltimos tilni tanlang.
...
Здравствуйте, ${msg.from.first_name}. Выберите языка`,
        ...
        await User.findByIdAndUpdate(user._id,{...user,action:'get_phone'},{new:true})
...
const get_phone = async msg => {
    ...
    if (!user.phone_number){
        ...
        await User.findByIdAndUpdate(user._id,{...user,action:'ready'},{new:true})
        await welcome(msg)
    }
